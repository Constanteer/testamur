...
def test_graph_resource_constraint_does_not_stringify_object_evidence():
    ok, reasons, unresolved = graph_context_constraints_satisfied(
        {"resource": {"ref": "repo:one"}},
        source_ref="actor:one",
        target_ref="repo:one",
        unresolved=["resource"],
    )
    assert not ok
    assert reasons == ["resource_constraint_malformed_or_missing"]
    assert unresolved == []


def test_graph_principal_aliases_are_alternate_encodings_not_additive_grants():
    ok, reasons, unresolved = graph_context_constraints_satisfied(
        {"principal": "actor:one", "principals": ["actor:two"]},
        source_ref="actor:one",
        target_ref="repo:one",
        unresolved=["principal", "principals"],
    )
    assert not ok
...
def test_equivalent_graph_principal_aliases_are_accepted():
    ok, reasons, unresolved = graph_context_constraints_satisfied(
        {"principal": "actor:one", "principals": ["actor:one"]},
        source_ref="actor:one",
        target_ref="repo:one",
        unresolved=["principal", "principals"],
    )
...
def test_malformed_subject_alias_cannot_become_graph_authority_evidence():
    ok, reasons, unresolved = graph_context_constraints_satisfied(
        {"principal": "alias:operator"},
        source_ref="actor:one",
        target_ref="repo:one",
        source_subject={"principal": {"ref": "alias:operator"}},
...
def test_conflicting_subject_resource_aliases_fail_closed():
    ok, reasons, unresolved = graph_context_constraints_satisfied(
        ...
        source_ref="actor:one",
        target_ref="repo:one",
        target_subject={"resource": "repo:alias", "resources": ["repo:other"]},
        unresolved=["resource"],
    )
    assert not ok
