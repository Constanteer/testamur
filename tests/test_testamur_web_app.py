...
class TestamurCanonicalWebTest(unittest.TestCase):
    ...
    def test_static_surface_is_packaged_under_testamur_namespace(self) -> None:
        ...
        self.assertIn("comparePanel", app)
        self.assertIn("/v1/compare", app)
        self.assertIn("safeNextPath", app)
        self.assertIn("publicHostedRoute", app)
        self.assertIn("/signin?next=", app)
        self.assertIn("/verify-email-pending", app)
        self.assertIn("/v1/account/email-verification/verify", app)
        self.assertIn("/v1/account/email-verification/verify-code", app)
        self.assertIn('autocomplete="one-time-code"', app)
        self.assertIn("6-digit code", app)
        self.assertIn("submitVerificationCode", app)
        self.assertIn(".verification-code-input", styles)
        self.assertIn("/v1/account/email-verification/resend", app)
        self.assertIn("response.verification_required", app)
        self.assertIn("function authShell", app)
        ...
        self.assertIn("revalidationPanel", app)
        self.assertIn("Revalidation is where judgment belongs.", app)
        self.assertIn("This page does not manufacture an assessment", app)
        self.assertIn(".revalidation-layout", styles)
        self.assertIn(".compare-layout", styles)
        self.assertIn("projectPage", app)
        self.assertIn("Project overview", app)
        self.assertIn("object-about-primary", app)
        self.assertIn("Add first monitor", app)
        self.assertIn("upstream things your work depends on", app)
        self.assertNotIn('<span>Slug</span><strong>${esc(project.slug)}</strong>', app)
        self.assertIn(".object-about-primary", styles)
        self.assertIn("supplyChainPanel", app)
        self.assertIn("Supply chain", app)
        self.assertIn("manifest declaration != runtime use", app)
        self.assertIn("advisory identity match != affectedness verdict", app)
        self.assertIn("Advisory candidates", app)
        self.assertIn("assessment required", app)
        self.assertIn("intentionally not fuzzy-matched", app)
        self.assertIn("Dependency inventory", app)
        self.assertIn("data-supply-search", app)
        self.assertIn(".dependency-row", styles)
        self.assertIn(".manifest-row", styles)
        self.assertIn(".supply-chain-advisories", styles)
        self.assertIn(".supply-chain-layout", styles)
        ...
        self.assertIn("What should Testamur watch?", app)
        self.assertIn("monitor-submit-primary", app)
        self.assertIn("Record the first observation.", app)
        self.assertIn("monitor-baseline-primary", app)
        self.assertIn("recorded ≠ verified · fetched ≠ relied", app)
        self.assertIn(".monitor-baseline-card", styles)
        self.assertIn("Only when I ask", app)
        self.assertIn("a detected change is not an invalidity verdict", app)
        self.assertIn(".monitor-submit-primary", styles)
