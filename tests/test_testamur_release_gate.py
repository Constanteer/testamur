...
def test_release_audit_accepts_testamur_only_surface(tmp_path: Path) -> None:
    root = _fixture(
        tmp_path,
        include='"testamur*"',
        source="from .runtime_protocol import canonical_json\n",
    )
...
def test_release_audit_reports_witness_service_import(tmp_path: Path) -> None:
    ...
    assert any(
        item["code"] == "legacy_runtime_import"
        for item in result["violations"]
    )


def test_release_audit_reports_testamur_test_dependency_on_retired_runtime(
    tmp_path: Path,
) -> None:
    ...
    assert any(
        item["code"] == "legacy_runtime_test_import"
        for item in result["violations"]
    )


def test_release_audit_ignores_non_testamur_legacy_test_fixture(
    tmp_path: Path,
) -> None:
    root = _fixture(tmp_path, include='"testamur*"', source="VALUE = 1\n")
...
def test_release_audit_rejects_distribution_that_omits_testamur(
    tmp_path: Path,
) -> None:
    ...
    assert any(
        item["code"] == "canonical_runtime_package_missing"
        for item in result["violations"]
    )
