...
def _binding_evidence(value: Any) -> list[dict[str, Any]]:
    ...
    for raw in value:
        ...
        item["evidence_class"] = _required(
            item.get("evidence_class"), field="binding.evidence.evidence_class"
        )
        if item["evidence_class"] not in {"OBSERVED", "DERIVED", "DECLARED"}:
            raise ValueError(
                "binding.evidence.evidence_class must be OBSERVED, DERIVED, or DECLARED"
            )
        if item["evidence_class"] == "DERIVED":
            item["analyzer"] = _required(
                item.get("analyzer"), field="binding.evidence.analyzer"
            )
            item["analyzer_version"] = _required(
                item.get("analyzer_version"),
                field="binding.evidence.analyzer_version",
            )
        result.append(item)
    if not result:
...
def authority_reliance_impact(
    authority_result: Mapping[str, Any],
    reliance_store: RelianceReceiptView,
    *,
    scope_ref: str,
    ...
    Resource identity is never guessed from a display label or matching string.
    The caller must provide an evidence-bearing binding from an authority target
    to a canonical reliance object and/or exact revision.
    """
    ...
    for raw in bindings:
        ...
        target_ref = _required(
            binding.get("authority_target_ref"),
            field="binding.authority_target_ref",
        )
        ...
        if object_ref is None and revision_ref is None:
            raise ValueError(
                "authority/reliance binding requires object_ref and/or revision_ref"
            )
        normalized = {
            ...
            "object_ref": (
                ...
                else _required(object_ref, field="binding.object_ref")
            ),
            "revision_ref": (
                None
                if revision_ref is None
                else _required(revision_ref, field="binding.revision_ref")
            ),
            "evidence": _binding_evidence(binding.get("evidence")),
        }
        normalized_bindings.setdefault(target_ref, []).append(normalized)
    ...
    for raw_action in actions:
        ...
        target_ref = _required(
            action.get("target_ref"), field="authority_action.target_ref"
        )
            ...
            unmatched_actions.append(
                {
                    ...
                    "path_edge_ids": list(action.get("path_edge_ids") or []),
                    "reason": "no_explicit_authority_to_reliance_binding",
                }
            )
            continue
        ...
        for binding in target_bindings:
            for receipt in receipts:
                ...
                relied_object_ref = _required(
                    receipt.get("object_ref"), field="receipt.object_ref"
                )
                ...
                object_match = (
                    binding["object_ref"] is not None
                    and relied_object_ref == binding["object_ref"]
                )
                revision_match = (
                    binding["revision_ref"] is not None
                    and binding["revision_ref"]
                    in {str(value) for value in pinned.values()}
                )
                if not object_match and not revision_match:
                    continue
                ...
                key = (
                    ...
                    str(capability.get("resource") or ""),
                    path,
                )
                if key in seen:
                    continue
                ...
                impacts.append(
                    {
                        ...
                        "reliant_revision_ref": receipt.get("reliant_revision_ref"),
                        ...
                        "authority_path_edge_ids": list(path),
                        "authority_evidence_state": action.get("evidence_state"),
            ...
            unmatched_actions.append(
                {
                    ...
                    "capability": action.get("capability"),
                    "path_edge_ids": list(action.get("path_edge_ids") or []),
                    "reason": "explicit_binding_did_not_match_any_durable_reliance",
                }
            )

    impacts.sort(
        key=lambda item: (
            str(item["reliant_ref"]),
            str(item["receipt_id"]),
            str(item["authority_target_ref"]),
            str(item["capability"].get("namespace") or ""),
            str(item["capability"].get("action") or ""),
            tuple(item["authority_path_edge_ids"]),
        )
    )
    affected_reliants = sorted({str(item["reliant_ref"]) for item in impacts})

    return {
        "schema_version": "testamur.authority-reliance-impact.v1",
        "scope_ref": scope,
        "impact_count": len(impacts),
        "affected_reliant_refs": affected_reliants,
        "impacts": impacts,
        "unmatched_actions": unmatched_actions,
        "semantics": {
            "authority_does_not_imply_reliance": True,
            "resource_identity_is_never_guessed": True,
            "actual_durable_reliance_only": True,
            "reachable_does_not_mean_exercised": True,
            "authority_reachability_triggers_review_not_falsehood": True,
        },
    }
