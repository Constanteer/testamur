...
def explain_authority_path(
    ...
    *,
    ...
    support_ids = sorted(
        {
            _required(edge_id, field="supporting_edge_ids[]")
    ...
    for edge_id in support_ids:
        ...
        source_ref = str(edge["source_ref"])
        target_ref = str(edge["target_ref"])
        supporting_subject_refs.update({source_ref, target_ref})
    ...
    all_subject_refs.extend(
        ref for ref in sorted(supporting_subject_refs) if ref not in subject_refs
    )
