...
function supplyChainPanel(project, supplyChain) {
  ...
  const advisoryCandidates = supplyChain.advisory_candidates || [];
  ...
  return `<div class="supply-chain-layout">
    ...
    <section class="supply-inventory-card">
      <div class="supply-inventory-head">
        <div><h2>Dependency inventory</h2><p>Search the recorded declaration inventory. Open a row to inspect its persistent record and exact basis.</p></div>
      ...
      <div class="dependency-table" data-supply-list>${dependencyRows || '<div class="dependency-empty">No dependency records were projected from this scan.</div>'}</div>
    ...
    <section class="supply-chain-advisories">
      ...
      ${advisoryCandidates.length ? `<div class="advisory-candidate-list">${advisoryCandidates.map(item => {
        const matched = (item.matching_component_revision_ids || []).map(ref => dependencies.find(dep => dep.component_revision_id === ref)).filter(Boolean);
        const matchedText = matched.map(dep => `${dep.ecosystem || 'package'}:${dep.name || dep.component_revision_id}@${dep.version || '?'}`).join(', ');
        return `<div class="advisory-candidate-row">
          <span class="alert-mark">!</span>
          <span><strong>${esc(item.external_id || item.event_class || 'Advisory')}</strong><small>${esc(item.provider || 'provider')} · ${esc(matchedText || String((item.matching_component_revision_ids || []).length) + ' exact component revision match(es)')}</small></span>
          <span>${badge('assessment required', 'warn')}</span>
        </div>`;
      }).join('')}</div>` : '<div class="empty advisory-empty"><strong>No exact advisory candidates</strong><p>No recorded advisory currently has exact upstream revision refs overlapping this project inventory. This is not evidence that the project is unaffected.</p></div>'}
    ...
    <section class="supply-chain-boundary-card">
      ...
      <p>An advisory candidate is an exact identity overlap, not a vulnerability verdict for the built or deployed artifact. Applicability remains a separate recorded assessment.</p>
    </section>
  </div>`;
}
...
async function projectMonitorsPanel(project, monitors) {
  ...
  return `<div class="monitoring-layout">
    ...
    <section class="time-query-card">
      ...
      <div data-write-result></div>
      ${providers.length ? `<div class="provider-discovery provider-ready"><div><strong>Plugin monitor providers available</strong><span>${providers.map(name => esc(specs[name]?.label || name)).join(', ')}</span></div><a data-nav href="/integrations">Integration setup</a></div>` : `<div class="provider-discovery"><div><strong>Need repository or agent-aware targets?</strong><span>No plugin monitor providers are registered in this environment yet. The ordinary URL / locator monitor still works without plugins.</span></div><a data-nav class="btn btn-secondary btn-compact" href="/integrations">Install integrations</a></div>`}
    </section>
...
function quickstartPage() {
  shell(`<div class="guide-shell">
    ...
    <ol class="quickstart-steps">
      ...
      <li class="quickstart-optional"><div class="quickstart-number">+</div><div><h2>Optional: record the software supply chain</h2><p>For a code repository, run <code>testamur project import . --name &lt;project&gt;</code> from the repository root. Testamur records manifest bytes and dependency declarations separately from runtime use or vulnerability verdicts.</p><div class="quickstart-rules"><span>manifest declaration ≠ runtime use</span><span>advisory match ≠ affectedness</span></div></div></li>
    </ol>
...
async function statusPage() {
  ...
  try {
    ...
    const providers = status.extensions?.monitor_target_providers || [];
    const providerSpecs = status.extensions?.monitor_target_provider_specs || {};
    const integrationStatus = providers.length
      ? `<section class="environment-integration-strip ready"><div><span class="onboarding-kicker">INTEGRATIONS</span><strong>${providers.length} monitor provider${providers.length === 1 ? '' : 's'} registered</strong><small>${providers.map(name => esc(providerSpecs[name]?.label || name)).join(', ')}</small></div><a data-nav class="btn btn-secondary btn-compact" href="/integrations">Setup & usage</a></section>`
      : `<section class="environment-integration-strip"><div><span class="onboarding-kicker">INTEGRATIONS</span><strong>No plugin monitor providers registered</strong><small>Core URL monitoring is available. Add Codex/MCP integrations when you want host-specific provenance or provider-backed monitor targets.</small></div><a data-nav class="btn btn-secondary btn-compact" href="/integrations">Install integrations</a></section>`;
    shell(`<div class="page-title"><div><h1>Environment</h1><p>${esc(contextKind)} and available capabilities.</p></div>${badge(contextLabel, hosted ? 'neutral' : 'good')}</div>${integrationStatus}<div class="status-grid">${groups.map(([name, value]) => `<section class="status-card"><h2>${esc(name)}</h2><div class="status-rows">${Object.entries(value).map(([key, item]) => `<div><span>${esc(key.replaceAll('_', ' '))}</span><strong>${esc(typeof item === 'object' ? short(pretty(item), 100) : item)}</strong></div>`).join('')}</div></section>`).join('')}</div>`);
  } catch (error) {
